...
/* Activate this to get a perfect gaussian sor solver which may be a bit
 * slower during the pressure exchange communication */
//#define USE_FLIPPED_PRESSURECOMM
...
int main(int argn, char** args) {
    ...
    printf("MPI setup: processor name: %s, rank %d out of %d processors, il:%i ir:%i jb:%i jt:%i, rank_l:%d, rank_r:%d rank_b:%d rank_t:%d\n",
    		main_processor_name,rankOfMainProcess, nbrOfProcesses,il, ir,jb,jt, rank_l, rank_r, rank_b, rank_t);
	...
	while(t < t_end)
	{
		...
		if (verbose)
		{
			if (n_div!=0 && n % n_div == 0)
				printf(
						"process rank: %i step-counter: %i, time: %f, sor-interations: %i  outputfile: %s.%i.vtk \n",rankOfMainProcess,n, t, it, szProblem,n);
		}
	}

	timeEnd += clock() - timeStart;    // end
    timeEnd = timeEnd/CLOCKS_PER_SEC;  // rescale to seconds

	printf("Total runtime of process %i: %f seconds\n",rankOfMainProcess,timeEnd);
	...
	Programm_Stop("Process finished ...");
	MPI_Finalize();
	return -1;
}
